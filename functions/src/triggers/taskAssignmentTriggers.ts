...
export const assignTask = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (!caller.isHead && caller.role !== 'ceo') {
      throw new functions.https.HttpsError(
        'permission-denied',
        'فقط رؤساء الأقسام و CEO يمكنهم تعيين المهام'
      );
    }
 ...
 */
export const reassignTask = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'يجب تسجيل الدخول');
  }
  ...
  try {
    ...
    if (!caller.isHead && caller.role !== 'ceo') {
      throw new functions.https.HttpsError(
        'permission-denied',
        'فقط رؤساء الأقسام يمكنهم إعادة التعيين'
      );
    }
    ...
    const newAssignment = {
      ...
      completedAt: null,
      actualDuration: null,
      reassignReason: reason || 'غير محدد',
    };
